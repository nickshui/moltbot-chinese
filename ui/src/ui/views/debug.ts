...
export function renderDebug(props: DebugProps) {
  ...
  return html`
    <section class="grid grid-cols-2">
      <div class="card">
        ...
        <div class="stack" style="margin-top: 12px;">
          <div>
            <div class="muted">${t("common.status")}</div>
            ${securitySummary
              ? html`<div class="callout ${securityTone}" style="margin-top: 8px;">
                  ${t("debug.securityAudit")}: ${securityLabel}${info > 0 ? ` · ${info} ${t("debug.info")}` : ""}. ${t("debug.run")}
                  <span class="mono">moltbot security audit --deep</span> ${t("debug.forDetails")}
                </div>`
              : nothing}
            <pre class="code-block">${JSON.stringify(props.status ?? {}, null, 2)}</pre>
          </div>
          <div>
            <div class="muted">${t("status.health")}</div>
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">${t("debug.eventsDesc")}</div>
      ${props.eventLog.length === 0
        ? html`<div class="muted" style="margin-top: 12px;">${t("debug.noEventsYet")}</div>`
        : html`
