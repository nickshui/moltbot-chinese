  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nav.sessions")}</div>
...
function renderRow(
  ...
  const updated = row.updatedAt ? formatAgo(row.updatedAt) : t("common.na", "n/a");
      ...
      <div>
        ...
        >
          ${thinkLevels.map((level) =>
            html`<option value=${level}>${level || t("sessions.inherit", "inherit")}</option>`,
          )}
        </select>
      </div>
      <div>
